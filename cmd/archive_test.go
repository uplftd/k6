...
func TestArchiveNotContainsEnv(t *testing.T) {
	...
	// when we do archiving and passing the `--env` flags altogether with `--skip-env` flag
	testState.args = []string{"k6", "--env", "ENV1=lorem", "--env", "ENV2=ipsum", "archive", "--skip-env", fileName}
